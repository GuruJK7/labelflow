...
|---|---|
| **Columna `Tenant.codEnabled` en prod** (H7) | Antes de mergear/deployar: `SELECT column_name FROM information_schema.columns WHERE table_name='Tenant' AND column_name='codEnabled';` sobre `DIRECT_URL`. Inferencia: el worker de main ya lee `tenant.codEnabled` en cada job, así que muy probablemente está; **verificar igual**. Si falta: Adrian aplica `scripts/sql/cod-contrarreembolso.sql` (D9) **antes** del deploy; si no, el GET de settings rompe para todos. |
