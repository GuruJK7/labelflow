import { rastreoDeLabel } from '../transportista';
...
interface EmailData {
  ...
  /**
   * Transportista del envío. Opcional: si no viene, se infiere del formato de la
   * guía, así que los llamadores que no lo pasan siguen funcionando igual.
   */
  carrier?: string | null;
}
...
export function buildShipmentEmailHtml(data: EmailData): string {
  ...
  // [03-sep-2026] Antes esto era la URL de DAC escrita a mano, sin la guía. Con
  // un envío de Correo el comprador recibía un link a un rastreador que no
  // conoce su código, y con reparto propio lo mismo.
  const rastreo = rastreoDeLabel(data.carrier, data.guia);
  const transportista = escapeHtml(rastreo.nombre);
